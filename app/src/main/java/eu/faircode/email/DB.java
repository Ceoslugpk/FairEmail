...
public abstract class DB extends RoomDatabase {
    ...
    private static RoomDatabase.Builder<DB> migrate(final Context context, RoomDatabase.Builder<DB> builder) {
        ...
        return builder
                ...
                }).addMigrations(new Migration(301, 302) {
                    @Override
                    public void migrate(@NonNull SupportSQLiteDatabase db) {
                        logMigration(startVersion, endVersion);
                        db.execSQL("ALTER TABLE `message` ADD COLUMN `assignee` TEXT");
                        db.execSQL("ALTER TABLE `message` ADD COLUMN `status` TEXT");
                        db.execSQL("CREATE INDEX `index_message_assignee` ON `message` (`assignee`)");
                        db.execSQL("CREATE INDEX `index_message_status` ON `message` (`status`)");
                    }
                }).addMigrations(new Migration(302, 303) {
                    @Override
                    public void migrate(@NonNull SupportSQLiteDatabase db) {
                        logMigration(startVersion, endVersion);
                        db.execSQL("ALTER TABLE `message` ADD COLUMN `recurrence_rule` TEXT");
                    }
